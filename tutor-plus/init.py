...
def copy_runtime_files(project_dir):
    ...
    for script_src in SCRIPTS_DIR.glob("*.py"):
        shutil.copy2(script_src, scripts_dst / script_src.name)
        ok(f"scripts/{script_src.name}")

    example_csv = SCRIPTS_DIR / "audio_list.example.csv"
    if example_csv.exists():
        shutil.copy2(example_csv, scripts_dst / example_csv.name)
        ok("scripts/audio_list.example.csv")

    requirements_src = SKILL_DIR / "requirements.txt"
    if requirements_src.exists():
        shutil.copy2(requirements_src, project_path / "requirements.txt")
        ok("requirements.txt")
...
"""
    ...
    if not csv_path.exists():
        ...
        info("  使用: python scripts/generate_tts.py audio_list.csv ./audio")
    else:
...
def main():
    ...
    # 3. 拷贝运行脚本
    copy_runtime_files(project_dir)
