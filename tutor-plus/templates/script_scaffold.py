...
常见问题：
- 渲染卡住：先确认音频文件能被本机播放器打开；必要时用 --no-audio 方式临时排查
...
class MathScene(Scene):
    ...
    def show_subtitle_timed(self, text, duration, position=DOWN * 3.5, fade_in_time=0.5, fade_out_time=0.5):
        ...
        wait_time = max(0, duration - fade_in_time - fade_out_time)
        if wait_time:
            self.wait(wait_time)
    ...
    def show_subtitle_with_audio(self, text, audio_duration, position=DOWN * 3.5):
        ...
        wait_time = max(0, audio_duration - 1.0)  # 预留退场时间
        if wait_time:
            self.wait(wait_time)
...
3. 每幕动画时长必须 >= 音频时长
4. 配音提到什么，画面就高亮什么
5. 使用 create_subtitle() 创建字幕，不要创建额外字幕类
6. 每幕开头调用 add_scene_audio(scene_num)，确保成片包含配音
